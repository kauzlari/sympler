...
GridMeter_Register<GridMeterScalarFourierSin> grid_meter_scalar_fourier_sin("ScalarFourierSin");


/*--- GridMeterScalarFourierSin ---*/

GridMeterScalarFourierSin::GridMeterScalarFourierSin(GridAverager *averager): GridMeterScalar(averager)
{
  init();
}


void GridMeterScalarFourierSin::init()
{
  m_properties.setClassName("ScalarFourierSin");

  m_properties.setDescription("Measure Fourier-sin component of a scalar field.");
...
void GridMeterScalarFourierSin::setup()
{
  GridMeterScalar::setup();

//   if(m_dir != 0 && m_dir != 1 && m_dir != 2)
//     throw gError("GridMeterScalarFourierSin::setup", "Attribute 'direction' has a forbidden value \"" + ObjToString(m_dir) + "\"! It must have value \"0\", \"1\" or \"2\" representing one of the cartesian directions.");

//   if(m_lambda == HUGE_VAL)
//     throw gError("GridMeterScalarFourierSin::setup","Please define attribute 'waveLength'!");
  ...
  if(abort)
    throw gError("GridMeterScalarFourierSin::setup","k-vector does not seem to be well defined by attribute 'kVec'.");
...
void GridMeterScalarFourierSin::measure(data_sp data)
{
  ...
  FOR_EACH_FREE_PARTICLE_C
    (phase, m_colour,
     ...
     if (n) {
       ...
       double e = __iSLFE->tag.doubleByOffset(m_scalar_o)*sin(m_kVec*__iSLFE->r);
