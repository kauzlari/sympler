...
void IntegratorVelocityVerletX::integratePosition(Particle* p, Cell* cell)
{
  ...
  cell->doCollision(p, p->r, vel, p->force[force_index], (IntegratorPosition*) this); 
//   MSG_DEBUG("IntegratorVelocityVerletX::integratePosition", "vel = " << p->tag.pointByOffset(m_v_offset) << ", slot = " << m_v_offset);
//   MSG_DEBUG("IntegratorVelocityVerletX::integratePosition", "pos before = " << p->r);
  ...
  p->r += p->dt * (p->tag.pointByOffset(m_v_offset) + 0.5 * p->dt * p->force[force_index]);  
//   MSG_DEBUG("IntegratorVelocityVerletX::integratePosition", "pos after = " << p->r);
}
