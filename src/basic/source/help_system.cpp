...
string expr_help_str
    (
     ...
     "  * \"[ai]\" is a \"Vector\" as defined above. The index \"i\", indicates that it appears in an expression for a pair of particles \"i\" and \"j\" and belongs to particle \"i\". It is stored for each PARTICLE. If the variable is to be introduced by the user, s-he will just write 'symbol = \"a\"' in the corresponding module, e.g., in a <ParticleScalar/>\n"
"  * \"{aj}\" is a \"Matrix\" as defined above. The index \"j\", indicates that it appears in an expression for a pair of particles \"i\" and \"j\" and belongs to particle \"j\". It is stored for each PARTICLE. If the variable is to be introduced by the user, s-he will just write 'symbol = \"a\"' in the corresponding module, e.g., in a <ParticleTensor/>\n"
     ...
     "   * abs(Scalar): self-explanatory, component-wise for non-scalars, e.g., \"abs({aj})\"\n"
     "   * acos(Scalar): self-explanatory, component-wise for non-scalars\n"
     "   * asin(Scalar): self-explanatory, component-wise for non-scalars\n"
     "     (NOTE: asin and acos require arguments in the interval [-1,1],\n"
     "      which is the responsibility of the user. Otherwise, simulation\n"
     ...
     "   * cosh(Scalar): self-explanatory, component-wise for non-scalars\n"
