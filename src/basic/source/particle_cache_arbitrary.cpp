...
void ParticleCacheArbitrary::setupOffset()
{
  ...
  { 
    ...
    {
      throw gError("ParticleCacheArbitrary::setup", className() + " reports: search for symbol failed. The message was " + err.message()); 
    }
  }
  else
  {
    ...
    if(Particle::s_tag_format[m_colour].attrExists(m_symbolName))
      throw gError("ParticleCacheArbitrary::setup", className() + " reports: Symbol " + m_symbolName + " already existing. Second definition is not allowed for 'overwrite = \"no\"'.");
    else 
