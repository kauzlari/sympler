...
bool PairArbitrary::findStage() {
  MSG_DEBUG("PairArbitrary::findStage for " + className(), " START: stage = " << m_stage);
  if (m_stage == -1) {
    // no symbols used?
    if (m_function.usedSymbols().empty()) {
      m_stage = 0;
      MSG_DEBUG("PairArbitrary::findStage for " + className(), " for symbol '" << m_symbolName << "': no symbols used: stage is now " << m_stage);
      return true;
    }
    ...
    if (m_oldSymbols != "---") {
      ...
      MSG_DEBUG("PairArbitrary::findStage",className()<<"m_oldSymbols is"<<working);
      while (run) {
	string cur;
	...
	if (toRemove.empty())
	  throw gError("PairArbitrary::findStage", "Unable to find old symbol '" + cur + "' among used symbols");
      }
    }
    ...
    for (typed_value_list_t::const_iterator s = usedSymbols.begin(); s
      ...
      string name = (*s)->name();
      MSG_DEBUG("PairArbitrary::findStage for " + className(), ": now checking for used symbol with complete name " << name);
	...
	// remove "i" or "j"
	name.erase(name.size()-1, name.size()-1);
      MSG_DEBUG("PairArbitrary::findStage for " + className(), ":short name: " << name);
      ...
      MSG_DEBUG("PairArbitrary::findStage for " + className(), " for " << mySymbolName() << ": m_phaseUser = " << m_phaseUser);
		 ...
		 for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
		   {
			 ...
			 if(stage == -1)
			   {
			     MSG_DEBUG("PairArbitrary::findStage for " + className(), " for symbol '" << m_symbolName << "': too early because of " << (*vCIt)->className());
			     tooEarly = true;
			     m_stage = -1;
			   }
			 else
	  ...
	  (
	   ...
	   for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
	       ...
	       if(*symIt == name)
		 {
		   ...
		   if(stage == -1)
		     {
		       MSG_DEBUG("PairArbitrary::findStage for " + className(), " for symbol '" << m_symbolName << "': too early because of " << (*__iFE)->className());
		       tooEarly = true;
		       m_stage = -1;
		     }
		   else
    ...
    if (m_stage == -1) {
      if (nothing) {
	m_stage = 0;
	MSG_DEBUG("PairArbitrary::findStage for " + className(), " for symbol '" << m_symbolName << "': stage is now " << m_stage);
	return true;
      } else
	return false;
    } else {
    ...
    MSG_DEBUG("PairArbitrary::findStage for " + className(), " for symbol '" << m_symbolName << "': stage was already " << m_stage);
    return true;
  }
}

bool PairArbitrary::findStage_0() {
  MSG_DEBUG("PairArbitrary::findStage_0 for " + className(),  " START: stage = " << m_stage);
  if (m_stage == -1) {
    // no symbols used?
    if (m_function.usedSymbols().empty()) {
      m_stage = 0;
      MSG_DEBUG("PairArbitrary::findStage_0 for " + className(),  " for symbol '" << m_symbolName << "': no symbols used: stage is now " << m_stage);
      return true;
    }
    // this is for aborting, when there is a Calculator, which has stage = -1 itself
    bool tooEarly = false;
	...
	for (typed_value_list_t::const_iterator s = toRemove.begin(); s
	  ...
	  usedSymbols.remove(*s);
	if (toRemove.empty())
	  throw gError("PairArbitrary::findStage_0", "Unable to find old symbol '" + cur + "' among used symbols");
      }
    }
    ...
    for (typed_value_list_t::const_iterator s = usedSymbols.begin(); s
      ...
      string name = (*s)->name();
      MSG_DEBUG("PairArbitrary::findStage_0 for " + className(),  ": now checking for used symbol with complete name " << name);
      // unfortunately, we have to remove the brackets for vectors and tensors
      if (name[0] == '{' || name[0] == '[') {
	// remove the first bracket
	name.erase(0, 1);
	...
	// remove "i" or "j"
	name.erase(name.size()-1, name.size()-1);
      MSG_DEBUG("PairArbitrary::findStage_0 for " + className(),  ":short name: " << name);
    ...
    if (m_stage == -1) {
      if (nothing) {
	m_stage = 0;
	MSG_DEBUG("PairArbitrary::findStage_0 for " + className(),  " for symbol '" << m_symbolName << "': stage is now " << m_stage);
	return true;
      } else
	return false;
    } else {
    ...
    MSG_DEBUG("PairArbitrary::findStage_0 for " + className(),  " for symbol '" << m_symbolName << "': stage was already " << m_stage);
    return true;
  }
}
