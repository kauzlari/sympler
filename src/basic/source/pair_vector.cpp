...
void PairVector::init() {
  ...
  STRINGPC
    (symbol, m_symbolName,
     "Name of the symbol for the vector.")
    ;
...
void PairVector::setup() {
  ...
  if (m_cutoff <= 0)
    throw gError("PairVector::setup", className() + ": Attribute 'cutoff' has no value > 0!");

  if (m_expression_string == "undefined")
    throw gError("PairVector::setup", className() + ": Attribute 'expression' has value \"undefined\"");
  
  MSG_DEBUG("PairVector::setup",className() << "  Cutoff used  = " << m_cutoff);
  MSG_DEBUG("PairVector::setup",className() << "  If all pairs used  = " << m_allPairs);
  MSG_DEBUG("PairVector::setup",className() << "  Symbol used  = " << m_symbolName);
  if (m_allPairs) {
    FOR_EACH_COLOUR_PAIR
      (
       ...
       MSG_DEBUG("PairVector::setup()",className() << " defined Cutoff = " << m_cutoff);
...
void /*pair<size_t, size_t>*/PairVector::setSlot(ColourPair* cp, size_t& slot,
						 bool oneProp) {
  
  m_slot = slot = cp->tagFormat().addAttribute
    ("PairVector_" + cp->toString(), DataFormat::POINT, false, "m_vector").offset;
  cout<<"****************PairScalar_" + cp->toString() <<endl;
}

bool PairVector::findStage() {
  MSG_DEBUG("PairVector::findStage", className() << " START: stage = " << m_stage);
  if (m_stage == -1) {
    // no symbols used?
    if (m_function.usedSymbols().empty()) {
      m_stage = 0;
      MSG_DEBUG("PairVector::findStage", className() << " for symbol '" << m_symbolName << "': no symbols used: stage is now " << m_stage);
      return true;
    }
    ...
    if (m_oldSymbols != "---") {
      ...
      MSG_DEBUG("PairVector::findStage",className()<<"m_oldSymbols is"<<working);
      while (run) {
	string cur;
	...
	if (toRemove.empty())
	  throw gError("PairVector::findStage", "Unable to find old symbol '" + cur + "' among used symbols");
      }
    }
    ...
    for (typed_value_list_t::const_iterator s = usedSymbols.begin(); s
      ...
      string name = (*s)->name();
      MSG_DEBUG("PairVector::findStage", className() << ": now checking for used symbol with complete name " << name);
	...
	// remove "i" or "j"
	name.erase(name.size()-1, name.size()-1);
      MSG_DEBUG("PairVector::findStage", className() << ":short name: " << name);
      ...
      MSG_DEBUG("PairVector::findStage", className() << " for " << mySymbolName() << ": m_phaseUser = " << m_phaseUser);
		 ...
		 for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
		   {
			 ...
			 if(stage == -1)
			   {
			     MSG_DEBUG("PairVector::findStage", className() << " for symbol '" << m_symbolName << "': too early because of " << (*vCIt)->className());
			     tooEarly = true;
			     m_stage = -1;
			   }
			 else
	...
	//             MSG_DEBUG("ValCalculatorArbitrary::findStage", "loop over ParticleCaches START");
	FOR_EACH
	  (
	   ...
	   for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
	       ...
	       if(*symIt == name)
		 {
		   ...
		   if(stage == -1)
		     {
		       MSG_DEBUG("PairVector::findStage", className() << " for symbol '" << m_symbolName << "': too early because of " << (*__iFE)->className());
		       tooEarly = true;
		       m_stage = -1;
		     }
		   else
    ...
    if (m_stage == -1) {
      if (nothing) {
	m_stage = 0;
	MSG_DEBUG("PairVector::findStage", className() << " for symbol '" << m_symbolName << "': stage is now " << m_stage);
	return true;
      } else
	return false;
    } else {
    ...
    MSG_DEBUG("PairVector::findStage", className() << " for symbol '" << m_symbolName << "': stage was already " << m_stage);
    return true;
  }
}

bool PairVector::findStage_0() {
  MSG_DEBUG("PairVector::findStage_0", className() << " START: stage = " << m_stage);
  if (m_stage == -1) {
    // no symbols used?
    if (m_function.usedSymbols().empty()) {
      m_stage = 0;
      MSG_DEBUG("PairVector::findStage_0", className() << " for symbol '" << m_symbolName << "': no symbols used: stage is now " << m_stage);
      return true;
    }
    // this is for aborting, when there is a Calculator, which has stage = -1 itself
    bool tooEarly = false;
	...
	for (typed_value_list_t::const_iterator s = toRemove.begin(); s
	  ...
	  usedSymbols.remove(*s);
	if (toRemove.empty())
	  throw gError("PairVector::findStage_0", "Unable to find old symbol '" + cur + "' among used symbols");
      }
    }
    ...
    for (typed_value_list_t::const_iterator s = usedSymbols.begin(); s
      ...
      string name = (*s)->name();
      MSG_DEBUG("PairVector::findStage_0", className() << ": now checking for used symbol with complete name " << name);
      // unfortunately, we have to remove the brackets for vectors and tensors
      if (name[0] == '{' || name[0] == '[') {
	// remove the first bracket
	name.erase(0, 1);
	...
	// remove "i" or "j"
	name.erase(name.size()-1, name.size()-1);
      MSG_DEBUG("PairVector::findStage_0", className() << ":short name: " << name);
    ...
    if (m_stage == -1) {
      if (nothing) {
	m_stage = 0;
	MSG_DEBUG("PairVector::findStage_0", className() << " for symbol '" << m_symbolName << "': stage is now " << m_stage);
	return true;
      } else
	return false;
    } else {
    ...
    MSG_DEBUG("PairVector::findStage_0", className() << " for symbol '" << m_symbolName << "': stage was already " << m_stage);
    return true;
  }
}
