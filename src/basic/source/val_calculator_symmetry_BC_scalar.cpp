...
{
  MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage","START: stage = " << m_stage);
  if(m_stage == -1)
  {
...
//      string name = (*s)->name();
//       MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", className() << ": now checking for used symbol with complete name " << name);
      ...
      MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage"," for " << mySymbolName() << ": m_phaseUser = " << m_phaseUser);
  ...
  //             MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", "symbols of VC: ");
  ...
  //               cout << *symIt << endl;
          ...
          for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
          {
            if(*symIt == m_gradientName || *symIt == m_scalarName)
            {
              nothing = false;
              int stage = (*vCIt)->stage();
              if(stage == -1) 
              {
                MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", className() << " for symbol '"  << m_symbolName << "': too early because of " << (*vCIt)->className());
                tooEarly = true;
                m_stage = -1;
              }
              else
...
//             if(m_phaseUser == 0) pCs = &(Particle::s_cached_flat_properties_0);
            //             MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", "loop over ParticleCaches START");
            FOR_EACH
                (
                vector<ParticleCache*>,
            (*pCs)/*Particle::s_cached_flat_properties*/,
              
//               MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", "loop over ParticleCaches. Now at = " << (*i)->mySymbolName() << " for symbol " << name);

              
            list<string> symbols = (*i)->mySymbolNames();
              
//               MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", "symbols of Cache: ");
...
/*              for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
            ...
            for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
            {
              if(*symIt == m_gradientName || *symIt == m_scalarName)
              {
                ...
                if(stage == -1) 
                {
                  MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", className() << " for symbol '"  << m_symbolName << "': too early because of " << (*i)->className());
                  tooEarly = true;
                  m_stage = -1;
                }
                else
...
//     } // end loop over symbols
    ...
    if(m_stage == -1)
    {
      if(nothing)
      {
        throw gError("ValCalculatorSymmetryBCScalar::findStage"," for symbol '"  + m_symbolName + "': Didn't find Calculators computing my input values '" + m_scalarName + "' and '" + m_gradientName + "'. Something seems to be messy. If you cannot find a reason then contact the programmers.");
      } 
      else return false;
    }
    else 
    {
      MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", className() << " for symbol '"  << m_symbolName << "': stage is now " << m_stage);
      return true;
    }
  } // end if(m_stage == -1)
  else 
  {
    MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage", className() << " for symbol '"  << m_symbolName << "': stage was already " << m_stage);
    return true;
  }
}
...
{
  MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0","START: stage = " << m_stage);
  if(m_stage == -1)
  {
...
//      string name = (*s)->name();
//       MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", className() << ": now checking for used symbol with complete name " << name);
      ...
      MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0"," for " << mySymbolName() << ": m_phaseUser = " << m_phaseUser);
  ...
  //             MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", "symbols of VC: ");
  ...
  //               cout << *symIt << endl;
          ...
          for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
          {
            if(*symIt == m_gradientName || *symIt == m_scalarName)
            {
              ...
              if(stage == -1) 
              {
                MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", className() << " for symbol '"  << m_symbolName << "': too early because of " << (*vCIt)->className());
                tooEarly = true;
                m_stage = -1;
              }
              else
      ...
      if(tooEarly)
      {
        ...
        --__cp;
          ...
          // we have to search now in the ParticleCaches
            vector<ParticleCache*>* pCs;
             if(m_phaseUser == 0) pCs = &(Particle::s_cached_flat_properties_0);
            //             MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", "loop over ParticleCaches START");
            FOR_EACH
                (
                vector<ParticleCache*>,
            (*pCs)/*Particle::s_cached_flat_properties*/,
              
//               MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", "loop over ParticleCaches. Now at = " << (*i)->mySymbolName() << " for symbol " << name);

              
            list<string> symbols = (*i)->mySymbolNames();
              
//               MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", "symbols of Cache: ");
...
/*              for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
            ...
            for(list<string>::iterator symIt = symbols.begin(); symIt != symbols.end(); ++symIt)
            {
              if(*symIt == m_gradientName || *symIt == m_scalarName)
              {
                ...
                if(stage == -1) 
                {
                  MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", className() << " for symbol '"  << m_symbolName << "': too early because of " << (*i)->className());
                  tooEarly = true;
                  m_stage = -1;
                }
                else
...
//     } // end loop over symbols
    ...
    if(m_stage == -1)
    {
      if(nothing)
      {
        throw gError("ValCalculatorSymmetryBCScalar::findStage_0"," for symbol '"  + m_symbolName + "': Didn't find Calculators computing my input values '" + m_scalarName + "' and '" + m_gradientName + "'. Something seems to be messy. If you cannot find a reason then contact the programmers.");
      } 
      else return false;
    }
    else 
    {
      MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", className() << " for symbol '"  << m_symbolName << "': stage is now " << m_stage);
      return true;
    }
  } // end if(m_stage == -1)
  else 
  {
    MSG_DEBUG("ValCalculatorSymmetryBCScalar::findStage_0", className() << " for symbol '"  << m_symbolName << "': stage was already " << m_stage);
    return true;
  }
}
