...
#ifndef __F_PARTICLE_VECTOR_MATRIX_H
#define __F_PARTICLE_VECTOR_MATRIX_H
...
/*!
 * Implementation of a per particle force F acting on a \a point_t stored in the tag. The force takes a square matrix K as user input and -\a m_factor *K operates on a vector of one user-defined \a point_t U (stored in the tag as well) of all particles of the user defined colour, i.e. F=-\a m_factor *K.U . This force represents for example a conservative force, if the matrix is a stiffness matrix and the vector is a displacement, or the force can also represent a friction if the matrix is a friction matrix and the vector some velocity. NOTE: The force will only work properly if no particles are created or destroyed! It is the user's responsibility to provide a square matrix of appropriate size, which must be in binary format. If addressed by four indices p1, d1, p2, d2 they will be used in exactly this (reverse) order as d2+p2*SPACE_DIMS+d1*SPACE_DIMS*Np+p1*SPACE_DIMS*Np*SPACE_DIMS, where p1, p2 are particle indices, d1, d2 are indexing cartesian coordinates, SPACE_DIMS is the number of dimensions and Np is the number of particles of the user-defined colour.
 */
class FParticleVectorMatrix : public FParticle
{
 ...
 public:
   ...
   */
    FParticleVectorMatrix(Simulation *simulation);

  /*!
     * Destructor
   */
    virtual ~FParticleVectorMatrix();
