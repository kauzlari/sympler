...
const Callable_Register<TriangleCGMDInterpolation2Points> triangle_CGMD_interpolation2points("TriangleCGMDInterpolation2Vectors");

TriangleCGMDInterpolation2Points::TriangleCGMDInterpolation2Points(Simulation* sim)
  : Callable(sim), m_zeroStored(false), m_working_mat(NULL), m_inverse(NULL), m_permutation(NULL) 
{
  init();
}

TriangleCGMDInterpolation2Points::~TriangleCGMDInterpolation2Points()
{
...
void TriangleCGMDInterpolation2Points::init()
{
  m_properties.setClassName("TriangleCGMDInterpolation2Vectors");
  
  m_properties.setDescription("A TriangleCGMDInterpolation2Vectors interpolates atomic displacements, velocities, forces, and two addtional use defined vectors on the nodes of a user-defined triangular mesh. Linear CGMD interpolation is used as described in Rudd&Broughton, Phys. Rev. B 72 144104 \n"
...
//   m_momentum2Name = "undefined";
  ...
  STRINGPC 
      (vector1, m_point1Name,
       "Name for the symbol of the node storing the 1st interpolated user-defined vector.");

  m_point1Name = "undefined";

  STRINGPC 
      (vector2, m_point2Name,
       "Name for the symbol of the node storing the 2nd interpolated user-defined vector.");

  m_point2Name = "undefined";

  STRINGPC 
      (inVector1, m_point1InName,
       "Name for the symbol of the particle storing the input for the 1st interpolated user-defined vector.");

  m_point1InName = "undefined";

  STRINGPC 
      (inVector2, m_point2InName,
       "Name for the symbol of the particle storing the input for the 2nd interpolated user-defined vector.");

  m_point2InName = "undefined";
...
//   m_force2Name = "undefined";
  ...
  BOOLPC 
      (writeShapeF, m_writeShapeF,
       "Should the the rectangular FEM-interpolation matrix be written to a file?");

  m_writeShapeF = false;

  INTPC
    (writeShapeFEvery, m_writeShapeFEvery, 0,
     "Writing interval for the rectangular FEM-interpolation matrix. Notice: Only the calls of this module are counted, NOT every time step! If transient = \"no\", there is only one call. Inactive if 'writeShapeF = \"no\"'");

  m_writeShapeFEvery = 1;

  STRINGPC(shapeFName, m_shapeFName, "File name for storing the rectangular FEM-interpolation matrix in binary format. A counter and the ending '.bin' are added automatically. Inactive if 'writeShapeF = \"no\"'");

  m_shapeFName = "shapeFNmatrix";
...
void TriangleCGMDInterpolation2Points::setup()
{
  m_shapeFStepCounter = 0;
  m_shapeFFileCounter = 0;
  ...
  if(m_filename == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "Attribute 'nameInputFile' has value \"undefined\".");

  if(m_nodeSpecies == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "Attribute 'nodeSpecies' has value \"undefined\".");

  m_nodeColour = M_MANAGER->getColour(m_nodeSpecies);

  if(m_pSpecies == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "Attribute 'pSpecies' has value \"undefined\".");

  m_pColour = M_MANAGER->getColour(m_pSpecies);

  if(m_targetSlotName == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'targetSlot' has value \"undefined\"");

  if(m_displacementName == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'displacement' has value \"undefined\"");

  if(m_momentum1Name == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'momentum1' has value \"undefined\"");

//   if(m_momentum2Name == "undefined")
//     throw gError("TriangleCGMDInterpolation2Points::setup", "'momentum2' has value \"undefined\"");

  if(m_force1Name == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'force1' has value \"undefined\"");

  if(m_point1Name == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'vector1' has value \"undefined\"");

  if(m_point2Name == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'vector2' has value \"undefined\"");

  if(m_point1InName == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'inVector1' has value \"undefined\"");

  if(m_point2InName == "undefined")
    throw gError("TriangleCGMDInterpolation2Points::setup", "'inVector2' has value \"undefined\"");

//   if(m_force2Name == "undefined")
//     throw gError("TriangleCGMDInterpolation2Points::setup", "'force2' has value \"undefined\"");
  ...
  // if the target-slot symbol doesn't yet exist, we add it
    ...
    // but if we use accidentally a m_targetSlotName which does not appear in the file containing the particles, this goes wrong! Hence there is a consistency check in TriangleCGMDInterpolation2Points::setupAfterParticleCreation 
  ...
  MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_targetSlotName << " = " << m_targetSlotOffset);
  }

  if(Particle::s_tag_format[m_nodeColour].attrExists(m_displacementName)) {
    throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_displacementName  + " for species " + m_nodeSpecies + " already exists!");
  }
  ...
  MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_displacementName << " = " << m_displacementOffset);
  }

  if(Particle::s_tag_format[m_nodeColour].attrExists(m_momentum1Name)) {
    throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_momentum1Name  + " for species " + m_nodeSpecies + " already exists!");
  }
  // if the symbol doesn't yet exist, we add it
  else {
    // We make it persistent and initialise to zero right before computation
  ...
  MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_momentum1Name << " = " << m_momentum1Offset);
  }

//   if(Particle::s_tag_format[m_nodeColour].attrExists(m_momentum2Name)) {
//     throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_momentum2Name  + " for species " + m_nodeSpecies + " already exists!");
//   }
//   // if the symbol doesn't yet exist, we add it
//   else {
//     // We make it persistent and initialise to zero right before computation
...
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_momentum2Name << " = " << m_momentum2Offset);
//   }

  if(Particle::s_tag_format[m_nodeColour].attrExists(m_force1Name)) {
    throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_force1Name  + " for species " + m_nodeSpecies + " already exists!");
  }
  // if the symbol doesn't yet exist, we add it
  else {
    // We make it persistent and initialise to zero right before computation
    m_force1Offset = Particle::s_tag_format[m_nodeColour].addAttribute(m_force1Name, DataFormat::POINT, true, m_force1Name).offset;
  MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_force1Name << " = " << m_force1Offset);
  }

  if(Particle::s_tag_format[m_nodeColour].attrExists(m_point1Name)) {
    throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_point1Name  + " for species " + m_nodeSpecies + " already exists!");
  }
  // if the symbol doesn't yet exist, we add it
  else {
    // We make it persistent and initialise to zero right before computation
    m_point1Offset = Particle::s_tag_format[m_nodeColour].addAttribute(m_point1Name, DataFormat::POINT, true, m_point1Name).offset;
  MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_point1Name << " = " << m_point1Offset);
  }

  if(Particle::s_tag_format[m_nodeColour].attrExists(m_point2Name)) {
    throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_point2Name  + " for species " + m_nodeSpecies + " already exists!");
  }
  // if the symbol doesn't yet exist, we add it
  else {
    // We make it persistent and initialise to zero right before computation
    m_point2Offset = Particle::s_tag_format[m_nodeColour].addAttribute(m_point2Name, DataFormat::POINT, true, m_point2Name).offset;
  MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_point2Name << " = " << m_point2Offset);
  }

  if(Particle::s_tag_format[m_pColour].attrExists(m_point1InName)) {
    m_point1InOffset = Particle::s_tag_format[m_pColour].indexOf(m_point1InName, DataFormat::POINT);
    m_point1InOffset = Particle::s_tag_format[m_pColour].offsetByIndex(m_point1InOffset);
  }
  // if the target-slot symbol doesn't yet exist, we add it
  else {
    throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_point1InName  + " for species " + m_nodeSpecies + " not found!");
  }

  if(Particle::s_tag_format[m_pColour].attrExists(m_point2InName)) {
    m_point2InOffset = Particle::s_tag_format[m_pColour].indexOf(m_point2InName, DataFormat::POINT);
    m_point2InOffset = Particle::s_tag_format[m_pColour].offsetByIndex(m_point2InOffset);
  }
  // if the target-slot symbol doesn't yet exist, we add it
  else {
    throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_point2InName  + " for species " + m_nodeSpecies + " not found!");
  }

//   if(Particle::s_tag_format[m_nodeColour].attrExists(m_force2Name)) {
//     throw gError("TriangleCGMDInterpolation2Points::setup", "Symbol " + m_force2Name  + " for species " + m_nodeSpecies + " already exists!");
//   }
//   // if the symbol doesn't yet exist, we add it
//   else {
//     // We make it persistent and initialise to zero right before computation
//     m_force2Offset = Particle::s_tag_format[m_nodeColour].addAttribute(m_force2Name, DataFormat::POINT, true, m_force2Name).offset;
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for " << m_force2Name << " = " << m_force2Offset);
//   }

  size_t __attrCounter = 0;
 
  while(Particle::s_tag_format[m_pColour].attrExists("__myElement" + ObjToString(__attrCounter))) {
    //     throw gError("TriangleCGMDInterpolation2Points::setup", "Attribute with name \"__myElement\" already created by other module. Cannot create twice. Avoid creating a symbol with this name in the input file!");
    ++__attrCounter;
  }
  ...
  MSG_DEBUG("TriangleCGMDInterpolation2Points::setup", "Offset for \"__myElement\"" << __attrCounter << " = " << m_targetSlotOffset);

  M_CONTROLLER->registerForSetupAfterParticleCreation(this);
}

void TriangleCGMDInterpolation2Points::setupAfterParticleCreation() 
{
...
//   oldParticleNode = newParticleNode;
  ...
  if(!different) 
    throw gError("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "All particles have the same targetSlot. Currently I consider this as an application error and abort. Check for example the attribute 'targetSlot'!"); 
...
//   m_inverse.data = NULL;
  ...
  if (!elementFile)
    throw gError("TriangleCGMDInterpolation2Points::setupAfterParticleCreation"+FILE_INFO,
...
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "Finished reading element nodes: nOfNodes = " << debug_nOfNodes); 
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "This is in my array of elements:");
...
// 	throw gError("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "That's bad! nOfNodes = " + ObjToString(debug_nOfNodes));
//     }
//     cout << endl;
//   }
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "again especially m_cornerIndices.at(element=864).at(node) = " << m_cornerIndices.at(864).at(0) << ", " << m_cornerIndices.at(864).at(1) << ", " << m_cornerIndices.at(864).at(2) );

//   MSG_DEBUG("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "NofNodes according to array m_elementsOfNode = " << debug_nOfNodes);
//   for(size_t node = 0; node < debug_nOfNodes; ++node) {
//     size_t debug_nOfElements = m_elementsOfNode[node].size();
//     MSG_DEBUG("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "nOfElements for node " << node << " = " << debug_nOfElements);
//     MSG_DEBUG("TriangleCGMDInterpolation2Points::setupAfterParticleCreation", "elements of node " << node << " : ");
...
// 	MSG_DEBUG
// 	  ("TriangleCGMDInterpolation2Points::setupAfterParticleCreation()", "START: periodicity check for element nodes for dir " << dir << endl 
...
MSG_DEBUG
  ("TriangleCGMDInterpolation2Points::setupAfterParticleCreation()", "END: periodicity of elements");
...
void TriangleCGMDInterpolation2Points::call(size_t timestep)
{
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "called for timestep " << timestep);
//   Phase* phase = M_PHASE;
  ...
  if(!m_zeroStored) {
    // 		  MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "!m_zeroStored case");
    compute();
    // currently not needed
  ...
  if (m_active) {
// 	MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "m_active == true case");
    if ((int) timestep == m_deactivate_at) {
      m_active = false;
// 	  MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "NOW DEACTIVATED");
    } 
    else {
      // 	  MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "still ACTIVE, m_step before reduct = " << m_step);
      m_step--;
      // 	  MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "m_step after reduct = " << m_step);
      if (!m_step) {
// 		MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "!m_step == true case");
	if(m_transient) {
	  // 		  MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "m_transient case");
	  compute();
	}
	// always interpolate
	interpolate();
    ...
    m_active = true;
// 	MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "now ACTIVATING, m_step before = " << m_step);
    m_step = m_interval;    
// 	MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "m_step after = " << m_step);
    if(m_transient) {
//       MSG_DEBUG("TriangleCGMDInterpolation2Points::call", "m_transient case");
      compute();
    }
...
void TriangleCGMDInterpolation2Points::compute()
{
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "called");
  Phase* phase = M_PHASE;
...
// 	   //----END:might vanish soon (2011-03-16)-------------
	 ...
	 break;
       ...
       ++debug_elit;
       if(debug_elit == endEl)
	 throw gError("TriangleCGMDInterpolation2Points::compute", "last element not matching! None found!");

     } // end of loop over elements
     );
  //--------------END: loop over particles -------------------------- 

  // write m_shapeF to file
  if(m_writeShapeF) {
    if(m_shapeFStepCounter == 0) {
      double* shapeFCopy = new double[nOfParticles*nOfNodes];
      for(size_t i = 0; i < nOfParticles; ++i) {
	for(size_t mu = 0; mu < nOfNodes; ++mu) {
	  shapeFCopy[mu+i*nOfNodes] = m_shapeF[i][mu];
	}}
      ofstream o;
      string fileName = string(m_shapeFName + ObjToString(m_shapeFFileCounter) + ".bin");
      o.open(fileName.c_str(), ios::out | ios::binary);
      o.write((char*) shapeFCopy, nOfParticles*nOfNodes*sizeof(double));
      o.close();
      delete[] shapeFCopy;
    }
    ++m_shapeFStepCounter;
    if(m_shapeFStepCounter == m_writeShapeFEvery) 
      m_shapeFStepCounter = 0;
    ++m_shapeFFileCounter;
  }
...
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "Matrix Nmunu:START:");
//   for(size_t i = 0; i < m_shapeMatrix.size(); ++i) {
//     for(size_t j = 0; j < m_shapeMatrix[i].size(); ++j)
//       cout << m_shapeMatrix[i][j] << " ";
//     cout << endl;
//   }
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "Matrix Nmunu:END");
...
//   std::time(&t1);
//   MSG_INFO("TriangleCGMDInterpolation2Points::compute", "matrix inversion: copy took: " << difftime(t1, t0) << " seconds");

  // Compute the LU decomposition
  int signum;
  int error = gsl_linalg_LU_decomp(m_working_mat, m_permutation, &signum);
  if(error)
    throw gError("TriangleCGMDInterpolation2Points::compute", "gsl_linalg_LU_decomp failed with return value " + ObjToString(error));

//   std::time(&t1);
//   MSG_INFO("TriangleCGMDInterpolation2Points::compute", "matrix inversion: LU took: " << difftime(t1, t0) << " seconds");
      
  // Compute the inverse
  error = gsl_linalg_LU_invert(m_working_mat, m_permutation, m_inverse);
  if(error)
    throw gError("TriangleCGMDInterpolation2Points::compute", "gsl_linalg_LU_invert failed with return value " + ObjToString(error));
  
//   std::time(&t1);
//   MSG_INFO("TriangleCGMDInterpolation2Points::compute", "matrix inversion took: " << difftime(t1, t0) << " seconds");

//   std::time(&t1);
//   MSG_INFO("TriangleCGMDInterpolation2Points::compute", "all about matrix inversion took: " << difftime(t1, t0) << " seconds");

//   cout << endl << endl << endl << endl << endl << endl 
//        << "#########################################################" 
//        << endl << endl << endl << endl << endl << endl;

//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "Matrix InverseNmunu:START:");
//   for(size_t i = 0; i < m_shapeMatrix.size(); ++i) {
//     for(size_t j = 0; j < m_shapeMatrix[i].size(); ++j)
//       cout << GSL_MATRIX_GET(m_inverse, i, j) << " ";
//     cout << endl;
//   }
//    MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "Matrix InverseNmunu:END");

  // -------- END: matrix inverse ------------------


//   size_t debug_nOfNodes = m_elementsOfNode.size();
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "Finished reading element nodes: nOfNodes = " << debug_nOfNodes); 
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "This is in my array of elements:");
//   for(size_t element = 0; element < m_cornerIndices.size(); ++element) {
//     for(size_t node = 0; node < 3; ++node) {
//       cout << m_cornerIndices.at(element).at(node) << " ";
//       if(m_cornerIndices.at(element).at(node) > debug_nOfNodes)
// 	throw gError("TriangleCGMDInterpolation2Points::compute", "AFTER matrix inverse: That's bad! nOfNodes = " + ObjToString(debug_nOfNodes));
//     }
//     cout << endl;
//   }

//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "element check after matrix inverse finished");
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "again especially m_cornerIndices.at(element=864).at(node) = " << m_cornerIndices.at(864).at(0) << ", " << m_cornerIndices.at(864).at(1) << ", " << m_cornerIndices.at(864).at(2) );
...
//   MSG_DEBUG("TriangleCGMDInterpolation2Points::compute", "FINISHED for timestep = " << timestep);

}

void TriangleCGMDInterpolation2Points::interpolate() const
{
  Phase* phase = M_PHASE;
  Controller* controller = M_CONTROLLER;

//    MSG_DEBUG("TriangleCGMDInterpolation2Points::interpolate", "CALLED at time " << controller->time());
...
//       node.tag.pointByOffset(m_force2Offset)[dir] = 0;
      ...
      node.tag.pointByOffset(m_point1Offset)[dir] = 0;
      node.tag.pointByOffset(m_point2Offset)[dir] = 0;
    }
  }
...
//     MSG_DEBUG("TriangleCGMDInterpolation2Points::interpolate", "i=" << i << endl << " momentum1-contrib=" << particles[i].v);
...
//        MSG_DEBUG("TriangleCGMDInterpolation2Points::interpolate", "mu=" << mu << ", i=" << i << endl << " momentum1-contrib=" << particles[i].v << endl << "weight=" << m_f[i][mu] << "momentum1 before:" << nodes[mu].tag.pointByOffset(m_momentum1Offset));

      node.tag.pointByOffset(m_momentum1Offset) +=
	// currently (2011-03-16) all masses=1!!! 
	m_f[i][mu]*p.v;

//        MSG_DEBUG("TriangleCGMDInterpolation2Points::interpolate", "momentum1 after:" << nodes[mu].tag.pointByOffset(m_momentum1Offset));

      // locally interpolated momentum IS COMPUTED ABOVE
      // non-locally interpolated force

//       if(timestep == 9)       
//  	MSG_DEBUG("TriangleCGMDInterpolation2Points::interpolate", "mu=" << mu << ", i=" << i << endl << " force1-contrib=" << p.force[forceIndex] << endl << "weight=" << m_f[i][mu] << "force1 before:" << nodes[mu].tag.pointByOffset(m_force1Offset));

      node.tag.pointByOffset(m_force1Offset) +=
	// currently (2011-03-16) all masses=1!!! 
	m_f[i][mu]*p.force[forceIndex];

      node.tag.pointByOffset(m_point1Offset) +=
	// currently (2011-03-16) all masses=1!!! 
	m_f[i][mu]*p.tag.pointByOffset(m_point1InOffset);

      node.tag.pointByOffset(m_point2Offset) +=
	// currently (2011-03-16) all masses=1!!! 
	m_f[i][mu]*p.tag.pointByOffset(m_point2InOffset);

//       if(timestep == 9)
//  	MSG_DEBUG("TriangleCGMDInterpolation2Points::interpolate", "force1 after:" << nodes[mu].tag.pointByOffset(m_force1Offset));
...
void TriangleCGMDInterpolation2Points::debugSumPerPart() const
{
  MSG_DEBUG("TriangleCGMDInterpolation2Points::debugSumPerPart", "Sum of contributions for");
...
void TriangleCGMDInterpolation2Points::debugSumPerNode() const
{
  MSG_DEBUG("TriangleCGMDInterpolation2Points::debugSumPerNode", "Sum of contributions for");
...
void TriangleCGMDInterpolation2Points::debugTotalSum() const
{
  MSG_DEBUG("TriangleCGMDInterpolation2Points::debugTotalSum", "Total sum:");
  MSG_DEBUG("TriangleCGMDInterpolation2Points::debugTotalSum", "sum_nodes->sum_particles:");
...
void TriangleCGMDInterpolation2Points::debugPrintM_F(size_t node) const
{
//   size_t pColour = M_MANAGER->getColour(m_pSpecies);
//   size_t nodeColour = M_MANAGER->getColour(m_nodeSpecies);
  ...
  MSG_DEBUG("TriangleCGMDInterpolation2Points::debugPrintM_F", "f_mui(" << node << "):START:");
  ...
  MSG_DEBUG("TriangleCGMDInterpolation2Points::debugPrintM_F", "f_mui(" << node << ")::END");
}
