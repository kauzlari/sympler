...
#ifndef __METER_AUTOCORRELATION_VECTOR_H
#define __METER_AUTOCORRELATION_VECTOR_H
...
/*!
 * This meter computes the autocorrelation funtion for an arbitrary vector-property stored in the \a Particle 's tag. Note that the autocorrelation of an average vector quantity of the WHOLE SYSTEM CANNOT be computed by this \a Meter !
 *
 * ASSUMPTIONS:
 * - constant timestep length
 *
 */
class MeterAutocorrelationVector: public Meter
{
protected:
  /*!
  ...
  /*!
   * memory offset to the input value for which the ACF should be computed 
   */
...
public:
   ...
   * Constructor
   */
  MeterAutocorrelationVector(Simulation *simulation);

  /*!
   * Destructor
   */
  virtual ~MeterAutocorrelationVector();
