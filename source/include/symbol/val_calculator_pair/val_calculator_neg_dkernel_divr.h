...
#endif
  {
    //    MSG_DEBUG("ValCalculatorNegDKernelDivr::compute", "BEFORE: tag-w=" << pD->tag.doubleByOffset(m_slot));
    
    point_t dummy = {{{0, 0, 0}}};
    pD->tag.doubleByOffset(m_slot) = m_wf->weight(pD, dummy, NULL);
    
    //    MSG_DEBUG("ValCalculatorNegDKernelDivr::compute", "AFTER: tag-w=" << pD->tag.doubleByOffset(m_slot) << ", m_wf-return=" << m_wf->weight(pD, dummy, NULL) << ", w-pairSlot=" << m_slot);    
  }
