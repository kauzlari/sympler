...
#ifndef __TRIANGLE_CGMD_INTERPOLATION_2POINTS_H
#define __TRIANGLE_CGMD_INTERPOLATION_2POINTS_H
...
/*!
 * A \a TriangleCGMDInterpolation interpolates atomic displacements, velocities, forces, and two additional user defined vectors on the nodes of a user-defined triangular mesh. Linear CGMD interpolation is used as described in Rudd&Broughton, Phys. Rev. B 72 144104
 ...
 */
class TriangleCGMDInterpolation2Points: public Callable {
 ...
 protected:
  ...
  /*!
   * Name of memory offset to the 1st additional interpolated point stored in the nodes
   */
  string m_point1Name;

  /*!
   * Name of memory offset to the 2nd additional interpolated point stored in the nodes
   */
  string m_point2Name;

  /*!
   * Name of memory offset to the input for the 1st additional interpolated point stored in the nodes
   */
  string m_point1InName;

  /*!
   * Name of memory offset to the input for the 2nd additional interpolated point stored in the nodes
   */
  string m_point2InName;
  ...
  /*!
   * Memory offset to the 1st additional interpolated point stored in the nodes
   */
  size_t m_point1Offset;

  /*!
   * Memory offset to the 2nd additional interpolated point stored in the nodes
   */
  size_t m_point2Offset;

  /*!
   * Memory offset to the input for the 1st additional interpolated point stored in the nodes
   */
  size_t m_point1InOffset;

  /*!
   * Memory offset to the input for the 2nd additional interpolated point stored in the nodes
   */
  size_t m_point2InOffset;
  ...
  /*!
   * Should the rectangular FEM-interpolation matrix be written to a file?
   */
  bool m_writeShapeF;

  /*!
   * Internal counter for deciding when to write the rectangular FEM-interpolation matrix
   */
  size_t m_shapeFStepCounter;

  /*!
   * Internal counter for the rectangular FEM-interpolation matrix files 
   */
  size_t m_shapeFFileCounter;

  /*!
   * User input for deciding how often to write the rectangular FEM-interpolation matrix
   */
  size_t m_writeShapeFEvery;

  /*!
   * File name for storing the FEM-interpolation matrix in binary format. A counter and the ending '.bin' are added automatically.
   */
  string m_shapeFName;
 ...
 public:
   ...
   */
  TriangleCGMDInterpolation2Points(Simulation* sim);
  
  /*!
   * Destructor
   */
  virtual ~TriangleCGMDInterpolation2Points();
